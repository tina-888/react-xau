...
const GoldMining: React.FC<GoldMiningProps> = ({ isLoggedIn, onLogin }) => {
  ...
  const handleSendData = async (e: React.FormEvent<HTMLFormElement>, cardType: "basic" | "standard" | "premium", miningPowerInput: string) => {
    e.preventDefault();
    setError(null);

    // Define `r` and `b` values based on the card type
    const { r, b } = cardType === "basic" ? { r: 0.002, b: 0.05 } : cardType === "standard" ? { r: 0.005, b: 0.1 } : { r: 0.01, b: 0.2 };

    const miningPowerValue = Number(miningPowerInput);
    if (isNaN(miningPowerValue) || miningPowerValue <= 0) {
      setError("Invalid mining power input.");
      return;
    }

    try {
      const calculatedResult = (miningPowerValue + (miningPowerValue * r) / 100 + (miningPowerValue * b) / 100) / 12;

      const data = {
        name: cardType,
        xau: Number(calculatedResult.toFixed(8)),
        usd: miningPowerValue,
        total: 0,
        status: "pending",
      };
  ...
  const renderCardBack = (cardType: "basic" | "standard" | "premium", setIsFlipped: React.Dispatch<React.SetStateAction<boolean>>, miningPowerInput: string, setMiningPowerInput: React.Dispatch<React.SetStateAction<string>>) => (
              ...
              <div className="grid gap-4">
                ...
                <span className="ml-2 text-white dark:text-white text-sm">
                  *1 USD = 0.00041089 XAU
                  <br />
                  *Return {cardType === "basic" ? "0.2%" : cardType === "standard" ? "0.5%" : "1%"} per day
                  <br />
                  *Bonus {cardType === "basic" ? "5%" : cardType === "standard" ? "10%" : "20%"}
                </span>
              </div>
              <div className="mt-4">
                ...
                <h1 className="text-white dark:text-white">
                  {(
                    (Number(miningPowerInput) +
                      (Number(miningPowerInput) * (cardType === "basic" ? 0.002 : cardType === "standard" ? 0.005 : 0.01)) / 100 +
                      (Number(miningPowerInput) * (cardType === "basic" ? 0.05 : cardType === "standard" ? 0.1 : 0.2)) / 100) /
                    12
                  ).toFixed(8)}{" "}
                  XAU = {miningPowerInput} USD
                </h1>
              </div>
              <div className="mt-4">
